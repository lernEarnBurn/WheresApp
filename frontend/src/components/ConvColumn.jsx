...
export function ConvColumn(props) {
  const { setRecipient, setMessages, setStatus, setConvId } =
  ...
  return (
    <motion.section className="overflow-hidden w-full h-full flex flex-col ">
      <div className="w-full h-[10%] py-2 px-5 flex justify-between items-center">
        {!props.profilePic ? (
          <div
            ...
            className="w-14 h-14 default-pic flex-shrink-0"
          ></div>
        ) : (
          <div
            onClick={props.displaySettings}
      ...
      <SearchBar
        ...
        backToConvDisplay={backToConvDisplay}
      />
      <div className="w-full h-[82%] flex flex-col overflow-y-scroll">
        {displayConvs ? (
          conversations.map((conv, index) => (
            <div
              onClick={() => setConvContext(conv, "exist")}
              key={index}
              className="w-full h-[15%] border-b py-3 px-3 border-gray-300 flex items-center gap-3 hover-bg"
            >
              ...
              <div className="flex flex-col overflow-hidden">
                ...
                <h2 className="text-gray-600 pr-2 truncate whitespace-nowrap">
                  {conv.lastMessage}
                </h2>
              </div>
            </div>
          ...
          contacts.map((contact, index) => (
            <div
              onClick={() => {
                setConvContext(contact, "new");
              }}
              key={index}
              className="w-full h-[15%] border-b py-4 px-3 border-gray-300 flex items-center gap-3 hover-bg"
            >
              {contact.profilePic ? (
                <div className="w-14 h-14 mx-1 rounded-full overflow-hidden flex-shrink-0">
                  <img
              ...
              <div className="flex flex-col mb-3 mt-1">
                ...
                <h2 className="text-gray-600 text-sm max-h-[2.6em] mr-2 overflow-hidden">
                  {contact.description}
                </h2>
              </div>
            </div>
    ...
    const convRequest = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/user/${user._id}/conversations`,
          {
            headers: {
        ...
        response.data.forEach((conv) => {
          ...
          if (conv.users[0].profilePic) {
            const picUrl = convertImageToUrl(
              conv.users[0].profilePic.image.data.data,
            );
