...
export function UserSettings(props) {
  ...
  const [profilePicUrl, setProfilePicUrl] = useState(
    props.profilePic ? props.profilePic : "public/images/stockAvatar.png",
  );
  ...
  return (
    ...
    >
      <div className="bg-color w-full h-[18%] flex gap-6">
        <ArrowLeft
          onClick={props.removeDisplaySettings}
          className="text-white mt-[5.3rem] ml-5"
        />
        <h1 className="text-white text-2xl mt-20 ml-5">Profile</h1>
      </div>
      <div className="w-full h-[82%] flex flex-col items-center">
        <div
          className="my-4 w-[12.5rem] h-[12.5rem] rounded-full overflow-hidden flex-shrink-0 cursor-pointer"
        ...
        <div className="w-full h-[6.5rem] user-input-bg mb-8">
          ...
          <input
            ...
            className="user-input-bg fg-color text-md w-[25.75vw] ml-6"
            type="text"
          />
        </div>
        <div className="w-full h-[6.5rem] user-input-bg">
          <h2 className="py-2 px-7 fg-color text-lg font-medium">
            Description
          </h2>
          <input
            value={description}
        ...
        {!loading ? (
          ...
          <button
            disabled
            className="w-[11vw] mt-5 px-5 h-11 text-md grid place-items-center"
          >
...
async function uploadProfilePic(setProfilePic, user, image) {
  try {
    ...
    const response = await axios
      .post(`http://localhost:3000/user/${user._id}/profilePic`, formData, {
        headers: {
